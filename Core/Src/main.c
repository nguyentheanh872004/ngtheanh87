...
uint8_t KeyPad_GetKey()
{
    for (uint8_t row = 0; row < KEYPAD_ROW; row++)
    {
        ...
        {
            if (HAL_GPIO_ReadPin(GPIOA, Keypad_ColPin[col]) == 1)
            {
                col_test = col;
                row_test = row;
    ...
    while (1)
    {
        ...
        if (key != 0)
        {
            ...
            case TINH:
                ...
                Lcd_gotoxy(0, 1);
                Lcd_write_int(ketqua);
                break;
            }
						
            Lcd_gotoxy(index++, 0);
						if(key >= '0' && key <= '9')
							Lcd_write_int(key - 48);
						else if(key == 'A')
							Lcd_write_string("+");
						else if(key == 'B')
							Lcd_write_string("-");
